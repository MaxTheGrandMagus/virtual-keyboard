...
class VirtualKeyboard {
  constructor() {
    ...
    for (let i = 0; i < this.langKeys[this.lang].length; i += 1) {
  ...
  handleBackspaceKey() {
    ...
    const actualPosition = this.input.selectionStart;
    ...
    if (actualPosition === endPosition) {
      this.handleInputText(prevText.slice(0, actualPosition - 1) + prevText.slice(endPosition));
      this.handleInputSelection(actualPosition - 1);
    } else {
      this.handleInputText(prevText.slice(0, actualPosition) + prevText.slice(endPosition));
      this.handleInputSelection(actualPosition);
    }
  }
  ...
  handleDeleteKey() {
    ...
    if (actualPosition === endPosition) {
      this.handleInputText(prevText.slice(0, actualPosition) + prevText.slice(actualPosition + 1));
      this.handleInputSelection(actualPosition);
    } else {
      this.handleInputText(prevText.slice(0, actualPosition) + prevText.slice(endPosition));
      this.handleInputSelection(actualPosition);
